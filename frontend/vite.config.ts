...
function getFontsConfig(): string {
  return (
    ...
    Object.keys(Fonts)
      .sort()
      .map((name: string) => {
        const config = Fonts[name as string];
  ...
  const prodPlugins: PluginOption[] = [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        runtimeCaching: [
          {
            urlPattern: (options) => {
              const isApi = options.url.hostname === "api.whitespaces.app";
    ...
    useSentry
      ? (sentryVitePlugin({
          ...
          applicationKey: "whitespaces-frontend",
        }) as Plugin)
      : null,
    replace([
      {
...
function getBuildOptions({
  ...
  return {
    ...
    rollupOptions: {
      input: {
        whitespaces: path.resolve(__dirname, "src/index.html"),
      ...
      output: {
        ...
        manualChunks: (id) => {
          ...
          if (id.includes("whitespaces/packages")) {
            return "whitespaces-packages";
          }
