...
import {
  LeaderboardEntry,
  XpLeaderboardEntry,
} from "@whitespaces/schemas/leaderboards";
...
import {
  PaginationQuery,
  FriendsOnlyQuery,
} from "@whitespaces/contracts/leaderboards";
import { Language, LanguageSchema } from "@whitespaces/schemas/languages";
import { isSafeNumber } from "@whitespaces/util/numbers";
import { Mode, Mode2, ModeSchema } from "@whitespaces/schemas/shared";
    ...
    requests = {
      data: async () => {
        ...
        if (response.ok) {
          ...
          return { status: 200, body: { data: { entries: data.data, count: data.data.length, pageSize: 10 } } };
        }
        return { status: 500, body: { message: "Failed to fetch leaderboard" } };
      },
      rank: undefined,
    };
