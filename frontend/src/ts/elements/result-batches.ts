...
import { mapRange } from "@whitespaces/util/numbers";
...
export async function update(): Promise<void> {
  ...
  if (results.length >= currentLimit) {
    ...
    //   text.html(
    //     `<br>Want to load up to ${limits?.premiumUser} results and gain access to more perks? Join Whitespaces Premium.<br>`
    //   );
    // }
  }
}
