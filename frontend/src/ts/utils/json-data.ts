import { Language, LanguageObject } from "@whitespaces/schemas/languages";
...
export async function getLatestReleaseFromGitHub(): Promise<string> {
  ...
  const releases = await cachedFetchJson<releaseType[]>(
    "https://api.github.com/repos/whitespacesgame/whitespaces/releases?per_page=1",
  );
...
export async function getReleasesFromGitHub(options?: {
  ...
  return fetchJson(
    `https://api.github.com/repos/whitespacesgame/whitespaces/releases?per_page=5&page=${options?.page ?? 1}`,
  );
}
