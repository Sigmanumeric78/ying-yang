...
function revert(): void {
  ...
  qs(".pageTest .ssWatermark")?.setText("whitespaces.app"); // Reset watermark text
...
async function generateCanvas(): Promise<HTMLCanvasElement | null> {
  ...
  const ssWatermark = [format(dateNow, "dd MMM yyyy HH:mm"), "whitespaces.app"];
...
export async function download(): Promise<void> {
  try {
    ...
    link.download = `whitespaces-result-${timestamp}.png`;
