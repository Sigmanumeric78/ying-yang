...
import * as Numbers from "@whitespaces/util/numbers";
...
import { QuoteLength, QuoteLengthConfig } from "@whitespaces/schemas/configs";
import { Mode } from "@whitespaces/schemas/shared";
import {
  CompletedEvent,
  CompletedEventCustomText,
} from "@whitespaces/schemas/results";
...
import { getFunbox } from "@whitespaces/funbox";
...
async function saveResult(
  ...
  const response = await fetch("http://localhost:3000/api/results", {
    ...
    body: JSON.stringify({
      ...
      mode: result.mode,
      characterStats: result.charStats,
    }),
  });
  ...
  if (!response.ok) {
    ...
    Notifications.add("Failed to save result", -1, { duration: 3 });
    console.error("Save result error:", errorData);
    return null;
  }

  const data = await response.json();
  Notifications.add("Result saved successfully", 1, { duration: 2 });

  const localDataToSave: DB.SaveLocalResultData = {};

  if (data.result !== undefined) {
    const snapshotResult = structuredClone(result) as unknown as SnapshotResult<Mode>;
    snapshotResult._id = data.result.id;
